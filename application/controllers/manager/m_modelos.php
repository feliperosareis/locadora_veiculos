 ...
 */
class m_modelos extends NT_Manager_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->crud->getState(), array('list', 'success')) && ($this->num_rows > 0 && $this->bUnique)) {
            redirect('/manager/modelos/index/edit/' . $this->id);
        }
        ...
        $this->crud ->set_rules($this->{$this->tabela}->getRules())
                    ...
                    ->set_subject("Modelos")
                    ->add_multiselect(base_url() .'manager/modelos/selecao_multipla/',true)
			        ->set_relation('ID_MARCA','nt_marcas','DESCRICAO')
			        ->add_bolean_status_switcher('STATUS', base_url().'manager/modelos/alterar_status/')
