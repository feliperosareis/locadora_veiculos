 ...
 */
class m_carros extends NT_Manager_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->crud->getState(), array('list', 'success')) && ($this->num_rows > 0 && $this->bUnique)) {
            redirect('/manager/carros/index/edit/' . $this->id);
        }
        ...
        $this->crud ->set_rules($this->{$this->tabela}->getRules())
                    ...
                    ->set_subject("Carros")
                    ->add_multiselect(base_url() .'manager/carros/selecao_multipla/',true)
			        ->add_bolean_status_switcher('STATUS', base_url().'manager/carros/alterar_status/')
                    ->set_relation('ID_MODELO', 'nt_modelos', 'DESCRICAO')	
