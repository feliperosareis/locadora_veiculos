 ...
 */
class m_clientes extends NT_Manager_Controller {
    ...
    public function index() {
        ...
        if (in_array($this->crud->getState(), array('list', 'success')) && ($this->num_rows > 0 && $this->bUnique)) {
            redirect('/manager/clientes/index/edit/' . $this->id);
        }
        ...
        $this->crud ->set_rules($this->{$this->tabela}->getRules())
                    ...
                    ->add_multiselect(base_url() .'manager/clientes/selecao_multipla/',true)
			        ->add_bolean_status_switcher('STATUS', base_url().'manager/clientes/alterar_status/')
        ...
        $data['jsexec'] = "$('#field-CPF').mask('000.000.000-00', { reverse: false });";
