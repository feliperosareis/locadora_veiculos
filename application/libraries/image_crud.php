...
class image_CRUD {
    ...
    //By Nissius// Gerar Miniatura no MultiUpload
    public $mini_altura = null;
    public $mini_largura = null;
    public $mini_tipo = 'inside';
    public $prefixo_miniatura = null;
    public $tinypng = true;
    public $miniaturas = null;
    public $field_adicional = null;
    public $value_field_adicional = null;
    
    public $type_img = null;
    public $field_type_img = null;
    ...
    function set_type_img($field,$valor) {
        $this->type_img = $valor;
        $this->field_type_img = $field;
        $this->_where($field, $valor);
    ...
    protected function _upload_file($upload_dir) {
        ...
        if (!empty($upload_response)) {
            ...
            $imagem = $upload_dir . '/' . $filename;

            //Se Estiver Ativo - Compactar com API da TinyPNG
            if ($this->tinypng == true) {
                $ci->load->library('compact_image');
                $ci->compact_image->tinypng($imagem);
            }
    ...
    protected function _where($field,$valor){
        $this->ci->db->where($field,$valor);
    }
    ...
    protected function _insert_table($file_name, $relation_id = null) {
        ...
        if (!empty($relation_id)){
            ...
            if(!is_null($this->field_type_img)){
                $insert[$this->field_type_img] = $this->type_img;
            }
        }
    ...
    protected function _delete_file($id) {
        ...
        unlink($this->image_path . '/mini_' . $result->{$this->url_field});
        if (!is_null($this->prefixo_miniatura)) {
            unlink($this->image_path . '/' . $this->prefixo_miniatura . $result->{$this->url_field});
        }
    ...
    protected function _get_delete_url($value) {
        $rsegments_array = $this->ci->uri->segment_array();
        return site_url($rsegments_array[1] . '/' . $rsegments_array[2] . '/' . $rsegments_array[3] . '/delete_file/' . $value);
    }
...
//        die('a');
        if ($this->getState()->name == 'upload_file') {
            if (($this->mini_altura > 0 and $this->mini_largura > 0) || is_array($this->miniaturas)) {
                $ci = &get_instance();
                $ci->load->library('wideimage');
                $formato = strtolower(pathinfo($thumbnail_path, PATHINFO_EXTENSION));
                if (is_array($this->miniaturas)) {
                    foreach ($this->miniaturas as $novos_tamanhos) {
                        $novo_arquivo = preg_replace('/thumb__/', $novos_tamanhos->prefixo_miniatura, $thumbnail_path, 1);
                        if ($formato == 'png' || $formato == 'jpg' || $formato == 'jpeg') {
                            $ci->wideimage->load($image_path)->resize($novos_tamanhos->mini_largura, $novos_tamanhos->mini_altura, $novos_tamanhos->mini_tipo)->saveToFile($novo_arquivo, ($formato == 'jpg' || $formato == 'jpeg' ? 95 : 9));
                            if ($novos_tamanhos->tinypng == true) {
                                //Se Estiver Ativo
                                $ci->load->library('compact_image');
                                $ci->compact_image->tinypng($novo_arquivo);
                            }
                        } else {
                            $ci->wideimage->load($image_path)->resize($novos_tamanhos->mini_largura, $novos_tamanhos->mini_altura, $novos_tamanhos->mini_tipo)->saveToFile($novo_arquivo, 1);
                        }
                    }
                } else {
                    $novo_arquivo = preg_replace('/thumb__/', $this->prefixo_miniatura, $thumbnail_path, 1);
                    if ($formato == 'png' || $formato == 'jpg' || $formato == 'jpeg') {
                        $ci->wideimage->load($image_path)->resize($this->mini_largura, $this->mini_altura, $this->mini_tipo)->saveToFile($novo_arquivo, ($formato == 'jpg' || $formato == 'jpeg' ? 95 : 9));
                        if ($this->tinypng == true) {
                            //Se Estiver Ativo
                            $ci->load->library('compact_image');
                            $ci->compact_image->tinypng($novo_arquivo);
                        ...
                        $ci->wideimage->load($image_path)->resize($this->mini_largura, $this->mini_altura, $this->mini_tipo)->saveToFile($novo_arquivo, 1);
                    }
                }
            }
        }
...
//        echo is_array($this->miniaturas);
    ...
    protected function getState() {
       ...
       if (isset($rsegments_array[3]) && is_numeric($rsegments_array[3])) {
            ...
            $file_name = substr(substr(rand(0,9999999), 9, 13) . '-' . $new_file_name, 0, 100);
            #endregion
...
class ImageUploadHandler {
    ...
    private function trim_file_name($name, $type) {
        ...
        $file_name = substr(rand(0,9999999), -5) . '-' . preg_replace("/([^a-zA-Z0-9\.\-\_]+?){1}/i", '-', $file_name);
