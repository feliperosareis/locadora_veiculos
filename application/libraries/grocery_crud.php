...
class grocery_CRUD_Field_Types {
    ...
    public function get_field_types() {
        ...
        foreach ($this->basic_model->get_field_types_basic_table() as $field_info) {
            ...
            } elseif ($field_info->name == 'ATIVO' or $field_info->name == 'ATIVA') {
                // regra própria nt_prime, campos ATIVO ou ATIVA são true_false por padrao
                $real_type = 'true_false';
                $field_info->crud_type = 'true_false';
            ...
            case 'upload_file':
                if (empty($value)) {
                    ...
                    $is_image = !empty($value) &&
                            ...
                            || strtolower(substr($value, -4)) == '.ico'
                            || strtolower(substr($value, -4)) == '.bmp'
                    ...
                    if ($is_image) {
                        $file_url_anchor .= ' class="image-thumbnail ext'.$extension.'"><img src="' . $file_url . '" height="50px">';
                    } else {
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types {
    ...
    private function delete_files_where_start_with($directory,$file){
        ...
        while (false !== ($entry = $d->read())) {
            ...
            if(is_file($path)){ // only delete if is a file, not a directory

                // if the file passed is a part of the current name, delete it
                if(preg_match("/".$file."/", $entry)){
                    unlink($path);
                }
            }
        ...
        if (isset($state_info->field_name) && isset($this->upload_fields[$state_info->field_name])) {
            ...
            if (file_exists("{$upload_info->upload_path}/{$state_info->file_name}")) {
                if (unlink("{$upload_info->upload_path}/{$state_info->file_name}")) {
                    ...
                    // correction suggested by Elias, delele all files that start with the original file name
                    // on the same directory as the original
                    $this->delete_files_where_start_with($upload_info->upload_path,$state_info->file_name);
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
    ...
    protected function showList($ajax = false, $state_info = null) {
        ...
        /* Implementacoes internas NoiaTec */
        $data->detailbox = $this->detailbox;
        $data->detailboxW = $this->detailboxW;
        $data->detailboxH = $this->detailboxH;
        $data->detailboxIcon = $this->detailboxIcon;
        $data->detailboxWindowTitle = $this->detailboxWindowTitle;

        $data->add_value = $this->add_value;


        $data->switcher = $this->switcher;
        $data->multiselect = $this->multiselect;
        $data->list_edit_order = $this->list_edit_order;

        $data->specialForID = $this->specialForID;

        /* fim implementacoes internas NoiaTec */
    ...
    /* Fix maxlenght issue in decimal fields */
    public function tup($value){
    ...
    protected function exportToExcel($state_info = null) {
        ...
        $indexControle=0;
        foreach($data->types as $key=>$fi){

            if(isset($fi->db_type)) {

                $este = new Fake();

                // sei que esse campo eh uma FK
                if(substr($fi->name,0,3) == "NT_"){
                    $nts_foreigns[$indexControle] = $fi->name;
                }

                $este->field_name = $fi->name;

                if (isset($fi->display_as))
                    $este->display_as = $fi->display_as;
                else
                    $este->display_as = $fi->name;

                $fake[$indexControle] = $este;
                $indexControle++;
            }
        }
        ...
        $initialFakeCount = count($fake);

        if(count($nts_foreigns) > 0){
            $plus=0;
            foreach($nts_foreigns as $fakeIndex=>$nameIndex){
                $plus++;

                // em data->list o campo de legenda Ã© o indeice ...
                $indiceReal = ($initialFakeCount + $plus);

                $interno=0;
                foreach ($data->list[0] as $nomeCampoKey => $naoImportra) {
                    $interno++;
                    if($interno == $indiceReal){
                        $fake[$fakeIndex]->field_name = $nomeCampoKey;
    ...
    protected function get_integer_input($field_info, $value) {
        ...
        if (!empty($field_info->db_max_length))
            $extra_attributes .= "maxlength='".$this->tup($field_info->db_max_length)."'";
    ...
    protected function get_text_input($field_info, $value) {
        if ($field_info->extras == 'text_editor') {
            ...
            elseif($this->config->text_editor_type == 'full'){
                $class_name = 'texteditor';
            }else{
                $class_name = $this->config->text_editor_type;
            }
    ...
    protected function get_readonly_input($field_info, $value) {
        if( preg_match('#^http:\/\/(.*)\.(gif|png|jpg|bmp|jpeg)$#i', $value)) {
            $value = "<img class='tamanho_foto_padrao' src='$value'><br>
                       $value";
        }
...
class grocery_CRUD extends grocery_CRUD_States {
    ...
    /* Implementacoes internas NoiaTec */
    protected $detailbox = false;
    protected $detailboxW = false;
    protected $detailboxH = false;
    protected $detailboxIcon = false;
    protected $detailboxWindowTitle = false;


    protected $switcher = false;
    protected $list_edit_order = false;
    protected $multiselect = false;
    protected $add_value = false;

    protected $specialForID = array();
    protected $tooltipdescription = array();
   ...
   /* fim implementacoes internas NoiaTec */
    ...
    /**
     * Passe o nome do field, e uma description, pode ter html no meio..
     * e isso vai ser exibido num tooltip no mouse over do nome do field.
     *
     * Use isso quando é preciso maiores explicações sobre o campo, valor, formato...
     *
     * @param type $field_name Nome do campo, como no BD
     * @param type $description Texto com HTMl se quiser, quebra de linha, negrito, imagem...
     * @return \grocery_CRUD
     */
    public function add_tooltip_description($field_name, $description){
        if(is_array($field_name)){
            foreach($field_name as $campos){
                $this->tooltipdescription[$campos] = $description;
            }
        }else{
            $this->tooltipdescription[$field_name] = $description;
        }
        return $this;
    }
    ...
    /**
     * Para alguns registros em específico podemos querer regras diferentes
     * Como por exemoplo o assunto padrão de um form de contato. Ele não pode ser
     * deletado, se for não vai mais funcionar a tela de contato.
     *
     * Neste método, passe o ID, e true/false para os argumentos
     *
     * Pode-se controlar se o registro vai ser listado ou não
     * Se ele vai poder ser editado ou não
     * e se ele vai poder ser deletado ou não.
     *
     * @param type $id
     * @param type $noList
     * @param type $noEdit
    ...
    public function special_for_id($id, $noList=false, $noEdit=false, $noDelete=false){

        $this->specialForID[$id] = array('noList'=>$noList,
                                         'noEdit'=>$noEdit,
                                         'noDelete'=>$noDelete);
        return $this;
    }
    ...
    /**
     * No CKEditor, quando tem a opção de figura e se quer permitir upload
     * deve se chamar essa função onformando qual é o diretorio em que
     * as imagens devem ficar.
     *
     * @param type $destinationPath string
     */
    public function set_field_editor_upload_folder($destinationPath){
        $ci = &get_instance();
        $ci->session->set_userdata("editor-upload-folder",$destinationPath);
    }



    /**
     * Adiciona uma coluna de ícone (na grid, lista) que permite trocar para o valor oposto o campo de status passado.
     * Importante que os valores possíveis sejam 0 ou 1. Opcionalmente, pode-se passar nos parametros
     * a figura que identifica aquele campo como false e como true.
     *
     * ATENÇÃO:
     * Testado com campos do tipo (no grocery) string, integer, dropdown e true_false.
     * Com valores 0 (zero, para false) ou 1(um, para true)
     *
     * @param type $field_name Nome do campo de status booleando
     * @param string $esteBooleanURL  URL para o método que implementa o switcher
     * @param string $true_img Caminho (preferencialmenete absoluto) da imagem que identifica como 1, true
     * @param string $false_img Caminho da imagem que indentifica para false
     * @return \grocery_CRUD
     */
    public function add_bolean_status_switcher($field_name, $esteBooleanURL, $true_img=null, $false_img=null){

        $this->switcher = true;

        $time_img = base_url()."assets/img/manager/status/time.gif";

        if(!$false_img)
            $false_img = base_url()."assets/img/manager/status/cancel.gif";

        if(!$true_img)
            $true_img = base_url()."assets/img/manager/status/accept.gif";

        $table_name = $this->basic_db_table;
        if(is_array($field_name)){
            foreach($field_name as $campos){
                //$this->tooltipdescription[$campos] = $description;
                $dis_as_legend = (isset($this->display_as[$campos]))?$this->display_as[$campos]:$campos;

                $image_url = $time_img."|".$true_img."|".$false_img."|".$esteBooleanURL;
                $link_url = base64_encode($campos."|".trim($table_name)."|".$dis_as_legend."|none");
                $link_url = urlencode($link_url);

                $this->add_action("Switcher", $image_url,rand(0,1000),$link_url);
            }
        }else{
            $dis_as_legend = (isset($this->display_as[$field_name]))?$this->display_as[$field_name]:$field_name;

            $image_url = $time_img."|".$true_img."|".$false_img."|".$esteBooleanURL;
            $link_url = base64_encode($field_name."|".trim($table_name)."|".$dis_as_legend."|none");
            $link_url = urlencode($link_url);
    ...
    /**
     * Adiciona input com opção de alterar os valores, na listagem do manager.
     *
     * ATENÇÃO:
     * O campo ORDEM não pode ter o nome no banco de dados = ORDER, pois é uma palavra reservada.
     *
     * @param type $field_name Nome do campo
     * @return \grocery_CRUD
     */

    public function add_list_edit_order($field_name,$actionURL, $legenda = 'Ordem'){
        // ATENÇÃO - Ele deverá verificar a obrigatoriedade do preenchimento pelas regras feitas na model.
        $this->list_edit_order = true;

        $table_name = $this->basic_db_table;

        //$image_url = $time_img."|".$true_img."|".$false_img;
        $link_url = base64_encode($field_name."|".trim($table_name)."|".$legenda."|".$actionURL."|none");
        $link_url = urlencode($link_url);
       // echo $link_url;
        $this->add_action("list_edit_order", '',rand(0,100),$link_url);

        return $this;
    }

    /**
     * Adiciona opções de checkbox no inicio da listagem,
     * para atualização de diferentes informação em "massa"!
     *
     * Se $onlySelectedOption for true, você terá uma session de nome composto por
     * <i>nome_da_tabela</i><b>-managerIDFilter</b> com o valor dos id's selecionados, separados por vírgula, "<i>1,5,556,12</i>" por exemplo
     *
     * @author Felipe Rosa <nissius@noiatec.com.br>
     * @author Helton Ritter <heltonritter@web.de>
     *
     * @param string $urlActuator URL para o método que implementa multiselect
     * @param boolean $delete  Deixar disponível a opção de deletar
     * @param array   $field_value  Array com nome dos campos e legenda: EXEMPLO: array('ATIVO' => 'Ativo);
     * @param boolen $onlySelectedOption Se quiser implementar um filtro pelos ID's dos itens selecionados, ponha para true.
     * @return \grocery_CRUD
     */
    public function add_multiselect($urlActuator, $delete = true, $fields = null, $onlySelectedOption=false){
        ...
        if(!$fields){
            ...
            $opcoes = '';
            foreach($fields as $campo => $legenda){
                $opcoes .= "$campo,$legenda#";
            }
            $opcoes = substr($opcoes,0,-1);
        }

        $link_url = base64_encode($delete."|".trim($table_name)."|".$opcoes."|".$onlySelectedOption."|".$urlActuator."|none");
        $link_url = urlencode($link_url);

        $this->add_action("multiselect", '',rand(0,100),$link_url);
     ...
     /**
     *
     * Adiciona o valor definido pelo usuário para um input -  By Nissius
     * @param $field  Nome da field IGUAL como se encontra no banco de DADOS.
     * @param $valor  Valor que deve vir preenchido no input.
     * @param $estatico - Opcional - Se o valor não pode ser alterado, coloque TRUE.
     */
    public function add_value($field,$valor,$estatico = false) {
        $this->add_value = true;

        $link_url = base64_encode($field."|".$valor."|".$estatico."|fix");
        $link_url = urlencode($link_url);
        $this->add_action("add_value", '',rand(0,10),$link_url);
    ...
    /**
     *
     * Adiciona breve descrição, na parte inferior do campo desejado -  By Nissius
     * @param $field  Nome da field IGUAL como se encontra no banco de DADOS.
     * @param $texto  Mensagem que deseja aparecer.
     * @param $cor  Cor em HEXADECIMAL, exemplo: #FFFFAA ou escrita: exemplo: RED.
     */
    public function add_comment($field,$texto,$cor = false) {
        if(is_array($field)){
            foreach($field as $campos){
                $this->comment[$campos] = "<div class='comentarios' ".(!empty($cor) ? "style='color: ".$cor."'" : "").">$texto</div>";
            }
        }else{
            $this->comment[$field] = "<div class='comentarios' ".(!empty($cor) ? "style='color: ".$cor."'" : "").">$texto</div>";
        }
        return $this;
    }
    ...
    /**
     * Abre uma determinada URL em um iframe. A URL que será aberta será
     * o valor indicado no param $url/ID, onde ID é a pk da linha em que foi clicado
     *
     * @param type $url url que deve abrir no iframe
     * @param int $w largura da janela fancy que deve abrir
     * @param int $h altura da janela fancy que deve abrir
     */
    public function add_detail_box($index,$url,$w,$h,$icon,$windowTitle) {
        $this->detailbox[$index] = $url;
        $this->detailboxW[$index] = $w;
        $this->detailboxH[$index] = $h;
        $this->detailboxIcon[$index] = $icon;
        $this->detailboxWindowTitle[$index] = $windowTitle;
    ...
    /**
     * Faz o re-label das campos conforme o que foi declarado
     * nas regras de validação de dados na model. Assim
     * não precisa re-declarar legendas na model e no controller,
     * apenas se a legenda que você quer não é a que está na model
     *
     * @param type $rules_set a model em ->getRules();
     * @return \grocery_CRUD retorno o objeto do grocery
     */

    public function auto_label($rules_set){

        foreach ($rules_set as $rule) {
            $this->display_as($rule['field'],$rule['label']);
        }
    ...
    function set_rules($field, $label = '', $rules = '') {
        ...
        if (is_string($field)) {
            ...
            foreach ($field as $num_field => $field_array) {
                ...
                if(preg_match("/required/", $field_array['rules']))
                    $requiredList[] = $field_array['field'];

            }

            $this->required_fields($requiredList);
        }
        return $this;
    }
    ...
    public function change_field_type($field, $type, $extras = null) {
        ...
        if (is_array($field)) {
            foreach ($field as $campos) {
                $this->change_field_type[$campos] = $field_type;
            }
        } else {
...
//@todo ui_day, ui_month, ui_year has to be lang strings
    ...
    /**
     * ATTENTION! Only on label (*) purpose on scren, that is NOT a required validation!
     * If you want to validade a field as required, call ->set_rules() method!
     *
    ...
    public function set_field_upload($field_name, $upload_dir = null) {
        ...
        if(is_array($field_name)){
            foreach($field_name as $campos){
                $this->upload_fields[$campos] = (object) array('field_name' => $campos, 'upload_path' => $upload_dir, 'encrypted_field_name' => $this->_unique_field_name($campos));
            }
        }else{
...
if (defined('CI_VERSION')) {
    ...
    class grocery_CRUD_Form_validation extends NT_Form_validation {
...
class UploadHandler {
    ...
    private function trim_file_name($name, $type) {
        ...
        $file_name = substr(rand(0,9999999), -5) . '-' . $this->_transliterate_characters($file_name);
